         ...
         if (this.description != null && !this.description.isBlank()) {
            if (this.minValue != null || this.maxValue != null) {
               ...
               if (minComparable.compareTo(this.maxValue) > 0) {
                  throw new IllegalArgumentException("Invalid option '" + this.path + "'. Min value " + this.minValue + " is greater than max " + this.maxValue);
               }
            }

         } else {
            throw new IllegalArgumentException("Invalid option name/path: " + this.path + "', description is missing or empty");
         }
      } else {
