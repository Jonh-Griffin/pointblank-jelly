...
public class DurabilityFeature extends ConditionalFeature {
   ...
   public MutableComponent getDescription() {
      return this.degradationMultiplier < (double)1.0F ? Component.translatable("description.pointblank.reducesDegradation").append(Component.literal(String.format(" %.0f%%", (double)100.0F * ((double)1.0F - this.degradationMultiplier)))) : Component.translatable("description.pointblank.increasesDegradation").append(Component.literal(String.format(" %.0f%%", (double)100.0F * (this.degradationMultiplier - (double)1.0F))));
   }
   ...
   public static void ensureDurability(ItemStack itemStack) {
      if (itemStack.getItem() instanceof GunItem) {
         ...
         for(Features.EnabledFeature enabledFeature : Features.getEnabledFeatures(itemStack, DurabilityFeature.class)) {
         ...
         if (tag != null) {
            ...
            if (currentMaxDurability != combinedMaxDurability) {
               ...
               currentDurability = (int)((float)currentDurability * Mth.clamp(diffRate, 0.0F, (float)Integer.MAX_VALUE));
   ...
   public static void degradeDurability(ItemStack itemStack) {
      ...
      if (tag != null) {
         ...
         durability = (int)Mth.clamp((double)durability * ((double)1.0F - degradationRate), 0.0F, currentMaxDurability);
   ...
   public static class Builder implements FeatureBuilder<Builder, DurabilityFeature> {
      ...
      public DurabilityFeature build(FeatureProvider featureProvider) {
         return new DurabilityFeature(featureProvider, this.condition, Mth.clamp(this.durability, 1, Integer.MAX_VALUE), Mth.clamp(this.degradationMultiplier, 1.0E-7, 1.0F));
      }
   }
}
