...
public class FireModeFeature extends ConditionalFeature {
   ...
   private FireModeFeature(FeatureProvider owner, Predicate<ConditionContext> predicate, List<FireModeInstance> fireModes) {
      ...
      for(FireModeInstance fireModeInstance : this.fireModeInstances) {
         ...
         for(Map.Entry<GunItem.FirePhase, List<Pair<Supplier<EffectBuilder<? extends EffectBuilder<?, ?>, ?>>, Predicate<ConditionContext>>>> e : fireModeInstanceEffectBuilders.entrySet()) {
            List<Pair<Supplier<EffectBuilder<? extends EffectBuilder<?, ?>, ?>>, Predicate<ConditionContext>>> firePhaseEffectBuilders = this.effectBuilders.computeIfAbsent(e.getKey(), (k) -> new ArrayList<>());
   ...
   public static class Builder implements FeatureBuilder<Builder, FireModeFeature> {
      ...
      public Builder withJsonObject(JsonObject obj) {
         ...
         for(JsonObject fireModeObj : JsonUtil.getJsonObjects(obj, "fireModes")) {
            ...
            fireModeBuilder.withDisplayName(Component.translatable(JsonUtil.getJsonString(fireModeObj, "displayName", name)));
            ...
            for(JsonObject completeFireAnimationObj : JsonUtil.getJsonObjects(fireModeObj, "completeFireAnimations")) {
               ...
               completeFireAnimationProvider.withAnimation(animationName, animationCondition, animationDuration, TimeUnit.MILLISECOND);
            }
