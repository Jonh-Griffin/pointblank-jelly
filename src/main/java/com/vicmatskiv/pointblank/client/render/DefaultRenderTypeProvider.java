...
public class DefaultRenderTypeProvider extends RenderStateShard implements RenderTypeProvider {
   ...
   private final Function<Boolean, RenderType> pipRenderTypes = Util.memoize(DefaultRenderTypeProvider::createPipRenderType);
   private final Function<ResourceLocation, RenderType> pipOverlayRenderTypes = Util.memoize((texture) -> createPipOverlayRenderType(texture, false));
   private final Function<ResourceLocation, RenderType> pipOverlayRenderTypesMasked = Util.memoize((texture) -> createPipOverlayRenderType(texture, true));
   private final Function<ResourceLocation, RenderType> pipMaskRenderTypes = Util.memoize(DefaultRenderTypeProvider::createPipMaskRenderType);
   ...
   private final Function<ResourceLocation, RenderType> reticleRenderTypes = Util.memoize((texture) -> createReticleRenderType(texture));
   ...
   private final Function<ResourceLocation, RenderType> glowEntityRenderTypes = Util.memoize(GlowEntityRenderType::createRenderType);
   ...
   public RenderType getGlowBlockEntityRenderType(ResourceLocation texture) {
      return IrisCompat.getInstance().isIrisLoaded() ? RenderType.entityTranslucent(texture) : this.glowEntityRenderTypes.apply(texture);
   }
   ...
   private static ShaderInstance getPosTexColorShader() {
      if (posTexColorShader != null) {
         ...
         try {
            ...
            posTexColorShader = Optional.of(new ShaderInstance(mc.getResourceManager(), shaderName, DefaultVertexFormat.POSITION_TEX_COLOR));
         } catch (Exception e) {
            e.printStackTrace();
   ...
   private static ShaderInstance getAuxShader() {
      if (auxShader != null) {
         ...
         try {
            Minecraft mc = Minecraft.getInstance();
            auxShader = Optional.of(new ShaderInstance(mc.getResourceManager(), shaderName, DefaultVertexFormat.POSITION_TEX_COLOR));
         } catch (Exception e) {
            e.printStackTrace();
   ...
   private static RenderType createPipRenderType(boolean isMasked) {
      ...
      return RenderTypeProvider.wrapRenderType(RenderType.create("pointblank:pip", DefaultVertexFormat.POSITION_TEX_COLOR, Mode.QUADS, 256, true, false, CompositeState.builder().setShaderState(new RenderStateShard.ShaderStateShard(DefaultRenderTypeProvider::getAuxShader)).setTextureState(PIP_TEXTURE_STATE_SHARD).setTransparencyState(NO_TRANSPARENCY).setDepthTestState(LEQUAL_DEPTH_TEST).setLightmapState(NO_LIGHTMAP).setOverlayState(NO_OVERLAY).setCullState(NO_CULL).createCompositeState(true)), setup, clear);
   }

   private static RenderType createPipMaskRenderType(ResourceLocation maskTexture) {
      return RenderTypeProvider.wrapRenderType(RenderType.CompositeRenderType.create("pointblank:pip_mask", DefaultVertexFormat.POSITION_TEX_COLOR, Mode.QUADS, 2097152, true, false, CompositeState.builder().setTransparencyState(NO_TRANSPARENCY).setTextureState(new RenderStateShard.TextureStateShard(maskTexture, false, false)).setShaderState(new RenderStateShard.ShaderStateShard(GameRenderer::getPositionTexColorShader)).createCompositeState(false)), RenderTypeProvider.SETUP_STENCIL_MASK_RENDER, RenderTypeProvider.CLEAR_STENCIL_MASK_RENDER);
   }
   ...
   private static RenderType createPipOverlayRenderType(ResourceLocation overlayTexture, boolean isMasked) {
      ...
      return RenderTypeProvider.wrapRenderType(RenderType.create("pointblank:pip_overlay_" + isMasked, DefaultVertexFormat.POSITION_TEX_COLOR, Mode.QUADS, 256, true, false, CompositeState.builder().setTransparencyState(RenderStateShard.TRANSLUCENT_TRANSPARENCY).setCullState(NO_CULL).setDepthTestState(LEQUAL_DEPTH_TEST).setLightmapState(NO_LIGHTMAP).setOverlayState(NO_OVERLAY).setTextureState(new RenderStateShard.TextureStateShard(overlayTexture, false, false)).setShaderState(new RenderStateShard.ShaderStateShard(DefaultRenderTypeProvider::getPosTexColorShader)).createCompositeState(false)), setup, clear);
   }
   ...
   private static RenderType createMuzzleFlashRenderType(ResourceLocation texture) {
      ...
      RenderType.CompositeState.CompositeStateBuilder var10006 = CompositeState.builder().setTransparencyState(LIGHTNING_TRANSPARENCY).setCullState(NO_CULL).setDepthTestState(LEQUAL_DEPTH_TEST).setLightmapState(LIGHTMAP).setOverlayState(OVERLAY).setTextureState(new RenderStateShard.TextureStateShard(texture, false, false));
      ClientSystem var10009 = ClientSystem.getInstance();
      Objects.requireNonNull(var10009);
      return RenderType.create("pointblank:muzzle_flash", var10001, var10002, 256, true, false, var10006.setShaderState(new RenderStateShard.ShaderStateShard(var10009::getColorTexLightmapShaderInstance)).createCompositeState(false));
   }
   ...
   private static RenderType createReticleRenderType(ResourceLocation texture, Supplier<ShaderInstance> shaderSupplier) {
      ...
      return RenderType.create("pointblank:reticle_parallax", var10001, var10002, 256, true, false, var10006.setShaderState(new RenderStateShard.ShaderStateShard(shaderSupplier)).createCompositeState(false));
   }
   ...
   private static RenderType createReticleRenderType(ResourceLocation reticleResource) {
      ...
      VertexFormat.Mode var10002 = Mode.QUADS;
      RenderType.CompositeState.CompositeStateBuilder var10006 = CompositeState.builder().setTransparencyState(TRANSLUCENT_TRANSPARENCY).setCullState(NO_CULL).setDepthTestState(LEQUAL_DEPTH_TEST).setLightmapState(LIGHTMAP).setOverlayState(OVERLAY).setTextureState(new RenderStateShard.TextureStateShard(reticleResource, false, false));
      ClientSystem var10009 = ClientSystem.getInstance();
      Objects.requireNonNull(var10009);
      return RenderType.create("pointblank:reticle", var10001, var10002, 256, true, false, var10006.setShaderState(new RenderStateShard.ShaderStateShard(var10009::getColorTexLightmapShaderInstance)).createCompositeState(false));
   }
   ...
   private static final class GlowEntityRenderType extends RenderType {
      ...
      private static RenderType createRenderType(ResourceLocation texture) {
         return RenderType.create("pointblank:glowy_entity_block", DefaultVertexFormat.POSITION_COLOR_TEX, Mode.QUADS, 256, true, false, CompositeState.builder().setTransparencyState(LIGHTNING_TRANSPARENCY).setCullState(NO_CULL).setLightmapState(LIGHTMAP).setOverlayState(OVERLAY).setTextureState(new RenderStateShard.TextureStateShard(texture, false, false)).setShaderState(RenderStateShard.POSITION_COLOR_TEX_SHADER).createCompositeState(false));
      }
   }
}
