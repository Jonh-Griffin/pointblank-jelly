...
public class SpriteEntityRenderer<T extends Entity & ProjectileLike> extends EntityRenderer<T> {
   ...
   public void render(T projectile, float p_114657_, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int lightColor) {
      ...
      if (uv != null) {
         ...
         for(int k = 0; k < this.brightness; ++k) {
            vertexConsumer.vertex(matrix4f, -halfWidth, halfHeight, 0.0F).color(1.0F, 1.0F, 1.0F, alpha).uv(maxU, minV).uv2(lightColor).endVertex();
            vertexConsumer.vertex(matrix4f, halfWidth, halfHeight, 0.0F).color(1.0F, 1.0F, 1.0F, alpha).uv(minU, minV).uv2(lightColor).endVertex();
            vertexConsumer.vertex(matrix4f, halfWidth, -halfHeight, 0.0F).color(1.0F, 1.0F, 1.0F, alpha).uv(minU, maxV).uv2(lightColor).endVertex();
            vertexConsumer.vertex(matrix4f, -halfWidth, -halfHeight, 0.0F).color(1.0F, 1.0F, 1.0F, (float)this.brightness).uv(maxU, maxV).uv2(lightColor).endVertex();
         }
   ...
   private static final class EffectRenderType extends RenderType {
      ...
      private static RenderType createRenderType(String name, ResourceLocation texture, Effect.BlendMode blendMode, boolean isDepthTestEnabled) {
         ...
         return RenderType.create(name, DefaultVertexFormat.POSITION_COLOR_TEX_LIGHTMAP, Mode.QUADS, 256, true, false, CompositeState.builder().setTransparencyState(transparencyShard).setDepthTestState(isDepthTestEnabled ? LEQUAL_DEPTH_TEST : NO_DEPTH_TEST).setCullState(NO_CULL).setLightmapState(LIGHTMAP).setOverlayState(OVERLAY).setTextureState(new RenderStateShard.TextureStateShard(texture, false, false)).setShaderState(RenderStateShard.POSITION_COLOR_TEX_LIGHTMAP_SHADER).createCompositeState(false));
      }
   }
   ...
   public static class Builder<T extends Entity & ProjectileLike> implements EntityRendererBuilder<Builder<T>, T, EntityRenderer<T>> {
      ...
      public EntityRenderer<T> build(EntityRendererProvider.Context context) {
         ...
         renderer.renderType = SpriteEntityRenderer.EffectRenderType.createRenderType(renderTypeName, this.texture, this.blendMode, this.isDepthTestEnabled);
         if (this.spriteInfo != null) {
            switch (this.spriteInfo.type()) {
               case STATIC:
                  ...
                  break;
               case LOOP:
                   renderer.spriteUVProvider = new LoopingSpriteUVProvider(this.spriteInfo.rows(), this.spriteInfo.columns(), this.spriteInfo.spritesPerSecond(), 1000L);
                  break;
               case RANDOM:
                  ...
                  break;
               case PLAY_ONCE:
                   renderer.spriteUVProvider = new PlayOnceSpriteUVProvider(this.spriteInfo.rows(), this.spriteInfo.columns(), this.spriteInfo.spritesPerSecond(), 1000L);
            }
         } else {
