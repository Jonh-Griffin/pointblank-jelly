...
public class TrailEffect extends AbstractEffect {
   ...
   public void render(EffectRenderContext effectRenderContext) {
      ...
      vertexConsumer.vertex(topRight.x, topRight.y, topRight.z).uv(u1, v1).color(rCol, gCol, bCol, alpha).uv2(lightColor).endVertex();
      vertexConsumer.vertex(bottomRight.x, bottomRight.y, bottomRight.z).uv(u1, v0).color(rCol, gCol, bCol, alpha).uv2(lightColor).endVertex();
      vertexConsumer.vertex(bottomLeft.x, bottomLeft.y, bottomLeft.z).uv(u0, v0).color(rCol, gCol, bCol, alpha).uv2(lightColor).endVertex();
      vertexConsumer.vertex(topLeft.x, topLeft.y, topLeft.z).uv(u0, v1).color(rCol, gCol, bCol, alpha).uv2(lightColor).endVertex();
   }
