...
public class AttachedProjectileEffect extends AbstractEffect {
   ...
   private boolean adjustPose(PoseStack poseStack) {
      ...
      if (currentItemPose != null) {
         ...
         Matrix4f modelMatrixAdjusted = (new Matrix4f()).mul(new Matrix4f(RenderUtil.getModelViewMatrixInverted())).mul(new Matrix4f(RenderUtil.getProjectionMatrixInverted())).mul(RenderUtil.getProjectionMatrixNormalFov()).mul(currentItemPose.pose());
   ...
   private void renderFace(Matrix4f transform, float zOffset, VertexConsumer vertexBuffer, float progress, int lightColor, SpriteUVProvider spriteUVProvider) {
      ...
      if (uv != null) {
         ...
         for(int k = 0; k < this.faceBrightness; ++k) {
            for(int i = 0; i < 4; ++i) {
               vertexBuffer.vertex(transform, positionOffsets[i][0], positionOffsets[i][1], positionOffsets[i][2]).uv(texUV[i][0], texUV[i][1]).color(1.0F, 1.0F, 1.0F, dynamicAlpha).uv2(lightColor).endVertex();
            }
         }
