...
public class RotationAnimationController extends AbstractProceduralAnimationController {
   ...
   public void onStateTick(LivingEntity player, GunClientState state) {
      ...
      if (newPhase != this.phase) {
         ...
         switch (newPhase) {
            case IDLE -> this.phaseDistanceFunction = new DecelerationDistance(previousVelocity, this.decelerationRate);
            case PREPARING -> this.phaseDistanceFunction = new AccelerationDistance(previousVelocity, this.accelerationRate);
            case RUNNING -> this.phaseDistanceFunction = new AccelerationDistance(previousVelocity, this.accelerationRate);
            case COMPLETING -> this.phaseDistanceFunction = new DecelerationDistance(previousVelocity, this.decelerationRate);
         }
   ...
   public void render(GunItemRenderer renderer, GunClientState gunClientState, PoseStack poseStack, GeoBone bone, VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
      ...
      poseStack.mulPose((new Quaternionf()).rotationXYZ(((float)Math.PI / 180F) * (float)((double)0.0F + (double)0.0F * this.progress), ((float)Math.PI / 180F) * (float)((double)0.0F + (double)0.0F * this.progress), ((float)Math.PI / 180F) * (float)((double)0.0F - currentDistanceInDegress)));
      poseStack.translate(-xPivot, -yPivot, -zPivot);
   ...
   private record AccelerationDistance(double initialVelocity, double acceleration) implements DistanceFunction {
      ...
      public double getDistance(double t) {
            ...
            return this.initialVelocity * tMax + (double) 0.5F * this.acceleration * tMax * tMax + distanceWithConstantSpeed;
         }
