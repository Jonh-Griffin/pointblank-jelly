...
public class GunRandomizingAnimationController extends AbstractProceduralAnimationController implements GunStateListener {
   ...
   private void reset(boolean isIdle, boolean isForward, long nanoSecPerTransition, double radius) {
      ...
      this.endRoll = -Math.signum(this.roll != 0.0D ? this.roll : 1.0D) * this.random.nextDouble() * this.currentAmplitude * 1.0D;
      this.endPitch = -Math.signum(this.pitch != 0.0D ? this.pitch : 1.0D) * this.random.nextDouble() * this.currentAmplitude * 0.5D;
      this.endYaw = -Math.signum(this.yaw != 0.0D ? this.yaw : 1.0D) * this.random.nextDouble() * this.currentAmplitude * 0.5D;
