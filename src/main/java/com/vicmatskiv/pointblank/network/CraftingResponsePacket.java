         ...
         switch (packet.result) {
            case COMPLETED:
               if (packet.isAddedToInventory) {
                  message = Component.translatable("").append(packet.itemStack.getItem().getName(packet.itemStack)).append(" ").append(Component.translatable("message.pointblank.added_to_the_inventory")).append("!");
               } else {
                  message = Component.translatable("").append(packet.itemStack.getItem().getName(packet.itemStack)).append(" ").append(Component.translatable("message.pointblank.dropped_on_the_gound")).append("!");
               }
               break;
            case FAILED:
               ...
               break;
            case CANCELLED:
               message = Component.translatable("").append(packet.itemStack.getItem().getName(packet.itemStack)).append(" ").append(Component.translatable("message.pointblank.cancelled_to_craft"));
         }
