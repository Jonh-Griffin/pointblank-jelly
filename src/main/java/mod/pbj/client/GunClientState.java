...
public class GunClientState {
   ...
   private final Predicate<GunClientStateContext> hasAmmo = (context) -> this.ammoCount.getAmmoCount(context.getFireModeInstance()) > 0;
   private final Predicate<GunClientStateContext> isValidGameMode = (context) -> context.player != null && !context.player.isSpectator();
   ...
   private StateMachine<FireState, GunClientStateContext> createStateMachine() {
      ...
      builder.withTransition(List.of(GunClientState.FireState.PREPARE_IDLE, GunClientState.FireState.IDLE, GunClientState.FireState.IDLE_COOLDOWN, GunClientState.FireState.INSPECT_COOLDOWN), GunClientState.FireState.PREPARE_FIRE_SINGLE, this.isValidGameMode.and(this.hasAmmo.and((context) -> GunItem.getSelectedFireModeType(context.itemStack) == FireMode.SINGLE || GunItem.getSelectedFireModeType(context.itemStack) == FireMode.MELEE).and((context) -> this.isTriggerOn && context.itemStack == context.player.getMainHandItem() && !context.player.isSprinting() && !FirstPersonWalkingAnimationHandler.isPlayingBlendingAnimation(Set.of(GunItem.RAW_ANIMATION_PREPARE_RUNNING, GunItem.RAW_ANIMATION_COMPLETE_RUNNING, GunItem.RAW_ANIMATION_RUNNING), context.itemStack, 0.0F, 0.25F))), TransitionMode.EVENT, null, this::actionPrepareFire);
