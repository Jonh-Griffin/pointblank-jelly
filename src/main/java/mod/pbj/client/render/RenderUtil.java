...
public class RenderUtil {
   ...
   public static void renderQuad(PoseStack poseStack, GeoQuad quad, VertexConsumer buffer, float dx, float dy, float red, float green, float blue, float alpha) {
      ...
      for(int i = 0; i < 4; ++i) {
         ...
         buffer.vertex(poseState, vertex.position().x, vertex.position().y, vertex.position().z).uv(u, v).color(red, green, blue, alpha).endVertex();
      }
   ...
   public static void renderQuadColorTex(PoseStack poseStack, GeoQuad quad, VertexConsumer buffer, float dx, float dy, float red, float green, float blue, float alpha) {
      ...
      for(int i = 0; i < 4; ++i) {
         ...
         float u = Mth.clamp(texUV[i][0], 0.0F, 1.0F);
         float v = Mth.clamp(texUV[i][1], 0.0F, 1.0F);
         buffer.vertex(poseState, vertex.position().x, vertex.position().y, vertex.position().z).color(red, green, blue, alpha).uv(u, v).overlayCoords(655360).uv2(15728640).normal(1.0F, 1.0F, 1.0F).endVertex();
      }
   ...
   public static void renderColoredQuad(PoseStack poseStack, GeoQuad quad, VertexConsumer buffer, float red, float green, float blue, float alpha) {
      ...
      for(int i = 0; i < 4; ++i) {
         ...
         buffer.vertex(poseState, v.position().x, v.position().y, v.position().z).color(red, green, blue, alpha).endVertex();
      }
   ...
   public static void blit(GuiGraphics guiGraphics, ResourceLocation resourceLocation, int startX, int endX, int startY, int endY, int zLevel, float minU, float maxU, float minV, float maxV, float colorR, float colorG, float colorB, float alpha) {
      ...
      bufferbuilder.vertex(matrix4f, (float)startX, (float)startY, (float)zLevel).uv(minU, minV).color(colorR, colorG, colorB, alpha).endVertex();
      bufferbuilder.vertex(matrix4f, (float)startX, (float)endY, (float)zLevel).uv(minU, maxV).color(colorR, colorG, colorB, alpha).endVertex();
      bufferbuilder.vertex(matrix4f, (float)endX, (float)endY, (float)zLevel).uv(maxU, maxV).color(colorR, colorG, colorB, alpha).endVertex();
      bufferbuilder.vertex(matrix4f, (float)endX, (float)startY, (float)zLevel).uv(maxU, minV).color(colorR, colorG, colorB, alpha).endVertex();
      BufferUploader.drawWithShader(bufferbuilder.end());
