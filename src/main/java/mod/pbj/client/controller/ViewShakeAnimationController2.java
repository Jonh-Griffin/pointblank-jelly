...
public class ViewShakeAnimationController2 extends AbstractProceduralAnimationController implements GunStateListener {
   ...
   public void onRenderTick(LivingEntity player, GunClientState gunClientState, ItemStack itemStack, ItemDisplayContext itemDisplayContext, float partialTicks) {
      ...
      if (!this.isDone) {
         double recoilPosZ = oscillatingFunction(this.progress * 10.0D, this.initialAmplitudeAdj, this.rateOfAmplitudeDecayAdj, this.initialAngularFrequencyAdj, this.rateOfFrequencyIncreaseAdj);
   ...
   public void reset() {
      ...
      this.initialAmplitudeAdj = this.initialAmplitude + (this.random.nextDouble() - 0.5D) * this.initialAmplitude * randomness;
      this.rateOfAmplitudeDecayAdj = this.rateOfAmplitudeDecay + (this.random.nextDouble() - 0.5D) * this.rateOfAmplitudeDecay * randomness;
      this.initialAngularFrequencyAdj = this.initialAngularFrequency + (this.random.nextDouble() - 0.5D) * this.initialAngularFrequency * randomness;
   ...
   public void reset(double initialAmplitude, double rateOfAmplitudeDecay, double initialAngularFrequency, double rateOfFrequencyIncrease) {
      ...
      this.rateOfAmplitudeDecayAdj = rateOfAmplitudeDecay + (this.random.nextDouble() - 0.5D) * rateOfAmplitudeDecay * randomness;
      this.initialAngularFrequencyAdj = initialAngularFrequency + (this.random.nextDouble() - 0.5D) * initialAngularFrequency * randomness;
      this.rateOfFrequencyIncreaseAdj = rateOfFrequencyIncrease + (this.random.nextDouble() - 0.5D) * rateOfFrequencyIncrease * 0.01D;
      super.reset();
   }
