...
public class AttachmentManagerScreen extends AbstractContainerScreen<AttachmentContainerMenu> {
	private static final ResourceLocation GUI_TEXTURES =
		new ResourceLocation("pointblank", "textures/gui/attachments4.png");
	private static final ResourceLocation BACKGROUND_TEXTURE =
		new ResourceLocation("pointblank", "textures/gui/blueprint-background-2.png");
	...
	private void renderItemInHand(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		BakedModel model = minecraft.getItemRenderer().getModel(
			itemStack,
			MiscUtil.getLevel(minecraft.player),
			minecraft.player,
			minecraft.player.getId() + ItemDisplayContext.GROUND.ordinal());
	...
	private void applyMouseInteractionTransforms(PoseStack poseStack, int mouseX, int mouseY) {
		float interactionOffsetX =
			(float)this.mouseInteractionHandler.getX() +
			(this.mouseInteractionHandler.isInteracting() && this.mouseInteractionHandler.isTranslating()
				 ? (float)mouseX - this.mouseInteractionHandler.getMouseClickedX()
				 : 0.0F);
		float interactionOffsetY =
			(float)this.mouseInteractionHandler.getY() +
			(this.mouseInteractionHandler.isInteracting() && this.mouseInteractionHandler.isTranslating()
				 ? (float)mouseY - this.mouseInteractionHandler.getMouseClickedY()
				 : 0.0F);
		poseStack.translate(interactionOffsetX, interactionOffsetY, 0.0F);
		float interactionPitch =
			this.mouseInteractionHandler.getRotationPitch() -
			(this.mouseInteractionHandler.isInteracting() && this.mouseInteractionHandler.isRotating()
				 ? (float)mouseY - this.mouseInteractionHandler.getMouseClickedY()
				 : 0.0F) -
			30.0F;
		float interactionYaw =
			this.mouseInteractionHandler.getRotationYaw() +
			(this.mouseInteractionHandler.isInteracting() && this.mouseInteractionHandler.isRotating()
				 ? (float)mouseX - this.mouseInteractionHandler.getMouseClickedX()
				 : 0.0F) +
			150.0F;
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		if (i instanceof AttachmentSlot attachmentSlot) {
			...
			if (childContainer != null) {
				...
				guiGraphics.renderOutline(
					attLeft + childContainerIndex * 22 - 2, attTop - 2, 22, 24 + 18 * elementsCount, -1061093377);
				poseStack.popPose();
			}
		}
		...
		for (int j = 0; j < attachmentContainers.length; ++j) {
			...
			if (attachmentContainer.getVirtualInventory() != null &&
				!attachmentContainer.getVirtualInventory().getElements().isEmpty()) {
				...
				for (int k = 1; k < attachmentContainer.getContainerSize(); ++k) {
					...
					if (slot.isActive()) {
						int textureTopOffset = this.menu.getCarried() != null && !this.menu.getCarried().isEmpty() &&
													   !slot.mayPlace(this.menu.getCarried())
												   ? unavailableSlotTextureTopOffset
												   : availableSlotTextureTopOffset;
						guiGraphics.blit(
							GUI_TEXTURES,
							attLeft + j * 22,
							attTop + this.headerBottomPadding + k * 18,
							1000,
							(float)slotTextureLeftOffset,
							(float)textureTopOffset,
							18,
							18,
							256,
							256);
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		return this.mouseInteractionHandler.onMouseButtonClicked(mouseX, mouseY, button) ||
			super.mouseClicked(mouseX, mouseY, button);
	}

	public boolean mouseReleased(double mouseX, double mouseY, int button) {
		return this.mouseInteractionHandler.onMouseButtonReleased(mouseX, mouseY, button) ||
	...
	private boolean isMouseInScreen(double mouseX, double mouseY) {
		...
		int left = this.getLeft();
		int top = this.getTop();
		boolean isMouseInInventory = mouseX >= (double)left && mouseX <= (double)(left + width) &&
									 mouseY >= (double)top && mouseY <= (double)(top + 90);
	...
	public Pair<RenderType, Vector4f> getRenderTypeOverride(
		ItemStack baseItemStack, ItemStack attachmentItemStack, String attachmentName, String attachmentPath) {
		...
		if (attachmentHighlight != null &&
			ItemStack.isSameItem(attachmentItemStack, attachmentHighlight.attachmentStack)) {
			...
			if (Objects.equals(attachmentPath, selectedAttachmentPath)) {
				...
				selectedAttachmentPath != null && selectedAttachmentPath.endsWith("/") &&
				attachmentPath.startsWith(selectedAttachmentPath.substring(0, selectedAttachmentPath.length() - 1))) {
				RenderType renderType = RenderType.eyes(texture);
				return Pair.of(renderType, COLOR_GREEN);
			} else {
				return null;
	...
	public void onAttachmentAdded(AttachmentAddedEvent event) {
		if (EffectiveSide.get() == LogicalSide.CLIENT) {
			...
			this.attachmentEventQueue.add(new AttachmentHighlightEvent(
				System.currentTimeMillis(), 750L, event.getParentStack(), event.getAttachmentStack()));
		}
	}
	...
	private static class AttachmentHighlightEvent {
		...
		public AttachmentHighlightEvent(
			long startTime, long duration, ItemStack parentStack, ItemStack attachmentStack) {
