...
public final class ScriptParser {
	...
	public static Script getScript(Path pathFromRun) {
		try {
			Script parse = new Script(
				pathFromRun.toString(),
				shell().compileReader(
					new FileReader(FMLPaths.GAMEDIR.get().resolve(pathFromRun).toFile()),
					pathFromRun.toString(),
					1,
					null),
	...
	public static Script getScript(Reader reader, String scriptName) {
		try {
			return new Script(
				scriptName,
				shell().compileReader(reader, scriptName, 1, null),
				shell().initStandardObjects(scope, true));
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	...
	public static Script cacheScript(Path toScript, ResourceLocation scriptId) {
		...
		try {
			Script script = new Script(
				scriptId.getPath(),
				shell().compileReader(
					new FileReader(FMLPaths.GAMEDIR.get().resolve(toScript).toFile()), scriptId.getPath(), 1, null),
				shell().initStandardObjects(scope, true));
	...
	public static Script cacheScript(Reader scriptReader, ResourceLocation scriptId) {
		...
		try {
			Script script = new Script(
				scriptId.getPath(),
				shell().compileReader(scriptReader, scriptId.getPath(), 1, null),
				shell().initStandardObjects(scope, true));
			SCRIPTCACHE.put(scriptId, script);
			return script;
		} catch (Exception e) {
