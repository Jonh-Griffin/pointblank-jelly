...
public class Expression {
   ...
   public static Predicate<ConditionContext> compile(String expression) {
      ...
      if (matcher.find()) {
         ...
         switch (op) {
            case "<" -> {
               return (context) -> tf.apply(context.player(), context.gunClientState(), context.currentItemStack()).doubleValue() < value;
            }
            case ">" -> {
               return (context) -> tf.apply(context.player(), context.gunClientState(), context.currentItemStack()).doubleValue() > value;
            }
   ...
   private static @NotNull TriFunction<LivingEntity, GunClientState, ItemStack, Number> expressClientStateMatcher(Matcher matcher) {
