...
public class TopDownAttackTrajectory implements Trajectory<TopDownAttackTrajectory.Phase> {
   ...
   public static TopDownAttackTrajectory createTrajectory(Vec3 startPosition, Vec3 targetPosition, double defaultSpeedBlocksPerSecond) {
      ...
      LOGGER.trace("Start of soft launch position: (%.5f, %.5f, %.5f)", startPosition.x, startPosition.y, startPosition.z);
      LOGGER.trace("End of soft launch position: (%.5f, %.5f, %.5f)", wouldBeClimbStartPosition.x, wouldBeClimbStartPosition.y, wouldBeClimbStartPosition.z);
      ...
      if (distanceToTarget < (double)25.0F) {
         ...
         if (climbInfo == null) {
            ...
            LOGGER.trace("Original normalized climb vector: (%.3f, %.3f, %.3f)", normalizedClimbVector.x, normalizedClimbVector.y, normalizedClimbVector.z);
            ...
            LOGGER.trace("Start of climb position: (%.5f, %.5f, %.5f)", wouldBeClimbStartPosition.x, wouldBeClimbStartPosition.y, wouldBeClimbStartPosition.z);
            LOGGER.trace("End of climb position: (%.5f, %.5f, %.5f)", topDownAttackTrajectory.endOfClimbPosition.x, topDownAttackTrajectory.endOfClimbPosition.y, topDownAttackTrajectory.endOfClimbPosition.z);
   ...
   private static void initSoftLaunch(TopDownAttackTrajectory topDownAttackTrajectory, ClimbInfo climbInfo) {
      ...
      topDownAttackTrajectory.startClimbHOffset = D * Math.cos(adjustedCurvature * (double)(n + 1) * (double)0.5F) + distanceToTurn;
   ...
   public void tick() {
      ...
      LOGGER.trace("Tick #%d, phase: %s, start: (%.5f, %.5f, %.5f), end: (%.5f, %.5f, %.5f), mv (%.5f, %.5f, %.5f)\n", this.currentTick, this.phase, this.startOfTickPosition.x, this.startOfTickPosition.y, this.startOfTickPosition.z, this.endOfTickPosition.x, this.endOfTickPosition.y, this.endOfTickPosition.z, this.deltaMovement.x, this.deltaMovement.y, this.deltaMovement.z);
      ++this.currentTick;
   }
   ...
   private Vec3 correctTrajectory() {
      ...
      if (this.verticalRotationAxis != null) {
         double rotationAxisSimilarity = newHorizontalRotationAxis.normalize().dot(this.verticalRotationAxis.normalize());
      ...
      rotation.rotateTo(this.deltaMovement.x, this.deltaMovement.y, this.deltaMovement.z, currentToTarget.x, currentToTarget.y, currentToTarget.z);
   ...
   private boolean checkTargetProximity() {
      ...
      if (startOfTickToTarget.lengthSqr() < magnitudeDeltaMovementSqr && endOfTickToTarget.lengthSqr() < magnitudeDeltaMovementSqr) {
   ...
   private void tickClimb() {
      ...
      this.deltaMovement = (new Vec3(this.climbVector.x, this.climbVector.y, this.climbVector.z)).normalize().scale(this.currentSpeed);
   ...
   private void tickTurn() {
      ...
      LOGGER.trace("Right: (%.5f, %.5f, %.5f)", this.verticalRotationAxis.x, this.verticalRotationAxis.y, this.verticalRotationAxis.z);
      if (targetDirOffsetDegrees < Math.toDegrees(this.climbToDescendCurvature)) {
         ...
         Quaterniond rotation = new Quaterniond();
         rotation.rotateAxis(rotationAngle, this.verticalRotationAxis.x, this.verticalRotationAxis.y, this.verticalRotationAxis.z);
         Vector3d dm = new Vector3d(this.deltaMovement.x, this.deltaMovement.y, this.deltaMovement.z);
         dm.rotate(rotation);
         this.deltaMovement = new Vec3(dm.x, dm.y, dm.z);
      }
   ...
   private void debugDeltaMovement() {
      ...
      LOGGER.trace("Direction to target: (%.3f, %.3f, %.3f)", directionToTarget.x, directionToTarget.y, directionToTarget.z);
   ...
   public void setTargetPosition(Vec3 updatedTargetPosition) {
      if (!(updatedTargetPosition.distanceToSqr(this.targetPosition) < 0.001)) {
         if (this.phase != TopDownAttackTrajectory.Phase.SOFT_LAUNCH && this.phase != TopDownAttackTrajectory.Phase.CLIMB_TO_DESCEND_TURN) {
            this.pendingCorrection = () -> {
               ...
               LOGGER.trace("Correction angle: %.3f, old right vector: (%.5f, %.5f, %.5f), new right vector: (%.5f, %.5f, %.5f)", Math.toDegrees(angle), currentRight.x, currentRight.y, currentRight.z, newRight.x, newRight.y, newRight.z);
               Quaterniond rotation = new Quaterniond();
               rotation.rotateAxis(angle, UP.x, UP.y, UP.z);
               Vector3d dm = new Vector3d(this.deltaMovement.x, this.deltaMovement.y, this.deltaMovement.z);
               dm.rotate(rotation);
               Vec3 updatedDeltaMovement = new Vec3(dm.x, dm.y, dm.z);
               LOGGER.trace("Updated delta movement from (%.5f, %.5f, %.5f) to (%.5f, %.5f, %.5f), angle: %.3f°", this.deltaMovement.x, this.deltaMovement.y, this.deltaMovement.z, updatedDeltaMovement.x, updatedDeltaMovement.y, updatedDeltaMovement.z, Math.toDegrees(angle));
   ...
   private void prepareClimb() {
      ...
      LOGGER.trace("Recomputed normalized climb vector: (%.3f, %.3f, %.3f)", normalizedClimbVector.x, normalizedClimbVector.y, normalizedClimbVector.z);
      ...
      if (climbAccelerationDistance > climbDistanceBeforeTurn) {
         double tickD = (-this.defaultSoftLaunchSpeed + Math.sqrt(this.defaultSoftLaunchSpeed * this.defaultSoftLaunchSpeed + (double)2.0F * this.climbAcceleration * climbDistanceBeforeTurn)) / this.climbAcceleration;
         climbAccelerationTicks = (int)Math.round(tickD);
      }

      Vec3 stopClimbAccelerationPos = this.endOfTickPosition.add(this.climbVector.normalize().scale(climbAccelerationDistance));
      LOGGER.trace("Climb acceleration distance: %.3f. Stop acceleration at: (%.3f, %.3f, %.3f)", climbAccelerationDistance, stopClimbAccelerationPos.x, stopClimbAccelerationPos.y, stopClimbAccelerationPos.z);
   ...
   public static ClimbInfo getClimbInfo(Vec3 source, Vec3 pivot, double radius) {
      ...
      if (launchAngle > (Math.PI / 2D)) {
         ...
         Vector3d v = (new Vector3d(sourceToPivot.x, sourceToPivot.y, sourceToPivot.z)).rotate(rotation).mul(Math.cos(alpha));
