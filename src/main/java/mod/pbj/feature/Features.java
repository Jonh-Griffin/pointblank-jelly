...
public class Features {
   ...
   private static List<EnabledFeature> computeEnabledFeatures(ItemStack rootStack, Class<? extends Feature> featureType) {
      ...
      for(Map.Entry<String, ItemStack> attachmentEntry : attachmentStacks.entrySet()) {
         ...
         if (item instanceof FeatureProvider fp) {
            ...
            if (feature != null && attachmentEntry.getValue().getItem() instanceof Attachment && feature.isEnabledForAttachment(rootStack, attachmentEntry.getValue())) {
   ...
   public static List<Supplier<EffectBuilder<? extends EffectBuilder<?, ?>, ?>>> getEnabledPhaseEffects(ItemStack itemStack, GunItem.FirePhase phase) {
      ...
      if (tag == null) {
         ...
         List<Pair<Supplier<EffectBuilder<? extends EffectBuilder<?, ?>, ?>>, Predicate<ConditionContext>>> conditionalEffects = enabledPhaseEffects.computeIfAbsent(Pair.of(tag, phase), (p) -> computeEnabledPhaseEffects(itemStack, phase));
      ...
      for(Class<? extends Feature> featureType : FeatureTypeRegistry.getFeatureTypes().values()) {
         for(Map.Entry<String, ItemStack> attachmentEntry : attachmentStacks.entrySet()) {
            ...
            if (item instanceof FeatureProvider fp) {
               Feature feature = fp.getFeature(featureType);
               if (feature != null && attachmentEntry.getValue().getItem() instanceof Attachment && feature.isEnabledForAttachment(rootStack, attachmentEntry.getValue())) {
                  List<Pair<Supplier<EffectBuilder<? extends EffectBuilder<?, ?>, ?>>, Predicate<ConditionContext>>> effectBuilders = feature.getEffectBuilders().get(firePhase);
